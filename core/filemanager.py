...
class FileManager:
    ...
    def load_yaml(self, file, *args, Loader=yaml.FullLoader, **kvargs):
        with open(file, "r") as f:
            data = list(yaml.load_all(f, *args, Loader=Loader, **kvargs))
            if len(data) == 1:
                data = data[0]
            return data

    def load_xls(self, file, *args, **kvargs):
        return pd.read_excel(file, *args, **kvargs)
